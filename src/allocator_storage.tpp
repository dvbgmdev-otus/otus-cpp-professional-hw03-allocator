...
AllocatorStorage<Capacity>::AllocatorStorage() noexcept
    ...
    ALLOCATOR_LOG(std::cout << "storage ctor: storage:" << this << '\n');
}

template <std::size_t Capacity>
AllocatorStorage<Capacity>::~AllocatorStorage() noexcept {
    ALLOCATOR_LOG(std::cout << "storage dtor: storage:" << this << " memory:" << m_memory
                               << " used:" << m_used << '\n');
...
U* AllocatorStorage<Capacity>::allocate(std::size_t n) {
    ...
    if (m_used + n > Capacity) {
        ALLOCATOR_LOG(std::cout << "storage allocate failed: type=" << typeid(U).name()
                                   << " n=" << n << " used=" << m_used << " capacity=" << Capacity
                                   << " storage:" << this << '\n');
        throw std::bad_alloc();
    }
    ...
    ALLOCATOR_LOG(std::cout << "storage allocate: type=" << typeid(U).name() << " n=" << n
                               << " used_before=" << m_used);
    m_used += n;
    ALLOCATOR_LOG(std::cout << " used_after=" << m_used << " from " << static_cast<void*>(result)
                               << " storage:" << this << '\n');
    return result;
}
...
void AllocatorStorage<Capacity>::ensureInitialized() {
    if (!m_initialized) {
        ...
        ALLOCATOR_LOG(std::cout
                         << "storage init: type=" << typeid(U).name() << " element_size="
                         << m_element_size << " element_align=" << m_element_align
                         << " capacity=" << Capacity << " memory:" << m_memory << " - "
    ...
    if (m_element_size != sizeof(U) || m_element_align != alignof(U)) {
        ALLOCATOR_LOG(std::cout
                         << "storage type mismatch: requested=" << typeid(U).name()
                         << " element_size=" << sizeof(U) << " element_align=" << alignof(U)
                         << " storage_size=" << m_element_size
                         << " storage_align=" << m_element_align << " storage:" << this << '\n');
        throw std::bad_alloc();
    }
}
